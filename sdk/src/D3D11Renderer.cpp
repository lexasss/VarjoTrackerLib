...
std::unique_ptr<Renderer::Texture> D3D11Renderer::createTexture2D(const glm::ivec2& resolution, varjo_TextureFormat format)
{
    DXGI_FORMAT d3dFormat;
    ...
    auto texture = std::make_unique<D3D11Renderer::Texture>();
    texture->init(resolution.x, resolution.y, TextureType::Texture2D);

    // Create texture and shader resource view.
    CD3D11_TEXTURE2D_DESC textureDesc(d3dFormat, resolution.x, resolution.y);
...
void D3D11Renderer::updateTexture(Renderer::Texture* texture, const uint8_t* data, size_t rowPitch)
{
    ...
    switch (d3d11Texture->getType()) {
        ...
        case TextureType::Texture2D: {
            ...
            if (rowPitch == mappedSubresource.RowPitch) {
                memcpy(mappedSubresource.pData, data, dataSize);
            } else {
                assert(mappedSubresource.RowPitch > rowPitch);
                size_t dstRowOffs = 0;
                size_t srcRowOffs = 0;
                for (int y = 0; y < d3d11Texture->getSize().y; y++) {
                    memcpy(reinterpret_cast<uint8_t*>(mappedSubresource.pData) + dstRowOffs, data + srcRowOffs, rowPitch);
                    dstRowOffs += mappedSubresource.RowPitch;
                    srcRowOffs += rowPitch;
                }
            }
